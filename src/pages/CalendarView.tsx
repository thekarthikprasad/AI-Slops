        ...
        <div className="animate-fade-in pb-24">
            ...
            <div className="p-4 space-y-6">
                ...
                <div className="card-gradient rounded-3xl p-4 shadow-ios">
                    ...
                    <div className="grid grid-cols-7 mb-2">
                        {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, i) => (
                            <div key={i} className="text-center text-xs font-medium text-white dark:text-ios-subtext ">
                                {day}
                            </div>
                        ))}
                    </div>
                            ...
                            return (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        isSelected ? "bg-ios-blue text-gray-900 shadow-md scale-105" : "hover:bg-ios-gray6",
                                        isToday && !isSelected && "text-ios-blue font-bold"
                                    )}
                                >
                                    <span className="text-sm text-gray-900 dark:text-white">{format(date, "d")}</span>
                                    {hasExpenses && (
                                        <div className={cn(
                                            "w-1 h-1 rounded-full mt-0.5",
                                            isSelected ? "bg-white/50" : "bg-ios-red"
                                        )} />
                                    )}
                                </button>
                            );
                ...
                <div className="space-y-4">
                    <h3 className="text-lg font-semibold px-2 text-gray-900 dark:text-gray-900 ">
                        {isSameDay(selectedDate, new Date()) ? "Today" : format(selectedDate, "EEEE, MMM d")}
                    </h3>

                    {selectedDayExpenses.length === 0 ? (
                        <div className="text-center py-8 text-gray-500 dark:text-ios-subtext card-gradient/50 rounded-2xl border border-dashed border-ios-gray3">
                        ...
                        <div className="space-y-3">
                            {selectedDayExpenses.map((expense) => (
                                ...
                                >
                                    <div className="flex gap-3 items-center">
                                        ...
                                        <div>
                                            <div className="font-medium text-gray-900 dark:text-gray-900 ">{expense.category}</div>
                                            {expense.note && (
                            ...
                            <div className="flex justify-between items-center px-4 pt-2 border-t border-ios-gray6">
                                <span className="text-sm font-medium text-ios-subtext text-gray-900 dark:text-white ">Total</span>
